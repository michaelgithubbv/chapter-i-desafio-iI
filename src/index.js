...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.todos.length === 10 && !user.pro)
    return response.status(403).json({ error: "user does not exist." });
...
function checksTodoExists(request, response, next) {
  ...
  const user = findUser(username);
  if (!user)
    return response.status(404).json({ error: "user does not exist.a" });

  const todoIndex = user.todos.findIndex((t) => t.id === id);
  if (todoIndex === -1)
    return response.status(404).json({ error: "user does not exist.aa" });
